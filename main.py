...
async def update_submenu(target_menu_id: str, target_submenu_id: str, submenu: schemas.SubmenusCreate,
...
@app.post("/api/v1/menus/{target_menu_id}/submenus/{target_submenu_id}/dishes",
          name="Создает блюдо",
          status_code=201)
async def create_dishe(target_menu_id: str, target_submenu_id: str, dish: schemas.DishesCreate,
                       db: Session = Depends(get_db)):
...
async def get_list_dishes(target_menu_id: str, target_submenu_id: str, target_dish_id: str,
                          db: Session = Depends(get_db)):
...
async def update_dish(target_menu_id: str, target_submenu_id: str, target_dish_id: str, dish: schemas.DishesCreate,
                      db: Session = Depends(get_db)):
    db_dish = crud.update_dish(db=db, menus_id=target_menu_id, submenu_id=target_submenu_id, dishes_id=target_dish_id,
                               dish=dish)
    if db_dish == None:
        raise HTTPException(status_code=404, detail="dish not found")
    return db_dish
